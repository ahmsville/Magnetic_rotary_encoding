...
int MagRotaryEncoder::processPULSE(int currentpos) {
	...
	{
		//CW
		if (digitalRead(get_sensorINTpin(3)) == 1) // state 3 - pin still high
		{
		...
		//CCW
		else if (digitalRead(get_sensorINTpin(1)) == 1) // state 1 - pin still high
		{
			processS2_N_INT = false; // disable yourown state
	...
	{
		//CW
		if (digitalRead(get_sensorINTpin(1)) == 1) // state 1 - pin still high
		{
		...
		//CCW
		else if (digitalRead(get_sensorINTpin(3)) == 1) // state 3 - pin still high
		{
			processS2_S_INT = false; // disable yourown state
			processS1_S_INT = false;
			processS2_N_INT = true;
	...
	{
		//CW
		if (digitalRead(get_sensorINTpin(2)) == 1) // state 2 - pin still high
		{
		...
		//CCW
		else if (digitalRead(get_sensorINTpin(4)) == 1) // state 4 - pin still high
		{
			processS1_S_INT = false; // disable yourown state
	...
	{
		//CW
		if (digitalRead(get_sensorINTpin(4)) == 1) // state 4 - pin still high
		{
		...
		//CCW
		else if (digitalRead(get_sensorINTpin(2)) == 1) // state 2 - pin still high
		{
			processS1_N_INT = false; // disable yourown state
			processS2_S_INT = false;
			processS2_N_INT = true;
...
void MagRotaryEncoder::DebugEncoder() {
	...
	//printf("\n");
	//printf("S1 %d S1_S %d S1_N %d S2 %d S2_S %d S2_N %d \n", get_sensorValue(1), digitalRead(get_sensorINTpin(1)), digitalRead(get_sensorINTpin(3)), get_sensorValue(2), digitalRead(get_sensorINTpin(2)), digitalRead(get_sensorINTpin(4)));
	//printf("\n");
	//
	outstr = String("prevstartposition>> ");
	outstr += prevstartposition;
	outstr += String(" startposition>> ");
	outstr += startposition;
	outstr += String(" abs_s1>> ");
	outstr += S1_absoluteneutral;
	outstr += String(" abs_s2>> ");
	outstr += S2_absoluteneutral;

	Serial.println(outstr);
	outstr = "";
	outstr += String(" stepres>> ");
	outstr += stepres;
	outstr += String(" setresolution>> ");
	outstr += setresolution;
	outstr += String(" bound>> ");
	outstr += bound;
	...
	outstr = "";
	outstr += String(" S1>> ");
	outstr += get_sensorValue(1);
	outstr += String(" S1_S>> ");
	outstr += digitalRead(get_sensorINTpin(1));
	outstr += String(" S1_N>> ");
	outstr += digitalRead(get_sensorINTpin(3));

	outstr += String(" S2>> ");
	outstr += get_sensorValue(2);
	outstr += String(" S2_S>> ");
	outstr += digitalRead(get_sensorINTpin(2));
	outstr += String(" S2_N>> ");
	outstr += digitalRead(get_sensorINTpin(4));
...
int MagRotaryEncoder::get_encodingState() {
	...
	{
		// printf("here %d \n",digitalRead(SensorINTpin[2]));
		// digital only detection
		if (digitalRead(SensorINTpin[2]) == 1)
		{

			if (digitalRead(SensorINTpin[1]) == digitalRead(SensorINTpin[3]))
			{
				return 3;
			}
		}
		else if (digitalRead(SensorINTpin[1]) == 1)
		{

			if (digitalRead(SensorINTpin[0]) == digitalRead(SensorINTpin[2]))
			{
				return 2;
			}
		}
		else if (digitalRead(SensorINTpin[0]) == 1)
		{
		...
		else if (digitalRead(SensorINTpin[3]) == 1)
		{
...
int MagRotaryEncoder::setToStart(int startfrom) {
	encoderPosition = startfrom;
...
int MagRotaryEncoder::get_bound(int sensor_num) // calculate ideal bound value for hall sensor
{
	...
	{
		Serial.print("\n");
		Serial.print("waiting for interrupt - rotate the knob...\n");
		while (digitalRead(get_sensorINTpin(sensor_num)) == 0) // wait for interrupt
		{
			/* code */
		}
		for (size_t i = 0; i < 10000; i++)
		{
			Serial.print("interrupt detected, reading sensor values\n");
			while (digitalRead(get_sensorINTpin(sensor_num)) == 1) // get upper and lower limit of center region for analog and digital detection
			{
	...
	Serial.print("\n");
	Serial.print("maximum value >> | minimum value >> ");
	Serial.print(maxval);
	Serial.print("\t");
	Serial.println(minval);
	int idealbound = maxval - minval;
	Serial.print("ideal bound >>");
	Serial.println(idealbound);

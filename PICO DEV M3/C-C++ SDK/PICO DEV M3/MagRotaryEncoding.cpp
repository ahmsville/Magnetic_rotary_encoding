...
{ // sets the start position based on the ADC values
	...
	{ // interrupt detection is not used
		int tempstartposition = get_encodingState();
		if (startposition != tempstartposition)
		{
			if (use_extended_resolution)
			{
				if (passSynctest(tempstartposition))
				{
					processCOUNT(startposition, tempstartposition);
					get_encResCount(count);
				}
			}
...
void MagRotaryEncoder::DebugEncoder()
{
	...
	printf("\n");
	printf("stepres >> %d setresolution >> %d bound >> %d encoderposition %d \n", stepres, setresolution, bound, encoderPosition);
	printf("\n");
...
void MagRotaryEncoder::setExtendedAbsolutePositioning(bool bres)
{
	exAbsolutePos = bres;
}

float MagRotaryEncoder::getinterpolatedStep(int upperlim, int lowerlim, int sdelta)
{
	float decimalpos = ((1 / float(abs(upperlim - lowerlim))) * float(sdelta));
	// limit to -1 to 1
...
float MagRotaryEncoder::detect_rotationHR()
{ // openloop rotation encoding function
	if (!useInterrupt || use_extended_resolution)
	{ // interrupt detection is not used
		get_sensorValue(1);
		get_sensorValue(2);
		// get difference in both sensors
		delta_sensor1 = sensor1 - S1_absoluteneutral;
		delta_sensor2 = sensor2 - S2_absoluteneutral;
		// return sign validated step change
		if (startposition == 1)
		{ // sensor2 is in neutral
			float retdec = getinterpolatedStep(S2_South, S2_absoluteneutral, delta_sensor2);
			if (retdec < 0 && encoderPosition == 1)
			{
				_interpolatedposition = float(encoderResolution) + retdec;
			}
			else
			{
				_interpolatedposition = float(encoderPosition - 1) + retdec;
			}
		}
		else if (startposition == 2)
		{ // sensor1 is in neutral
			float retdec = getinterpolatedStep(S1_South, S1_absoluteneutral, delta_sensor1);
			if (retdec < 0 && encoderPosition == 1)
			{
				_interpolatedposition = float(encoderResolution) + retdec;
			}
		...
		{ // sensor2 is in neutral
			...
			if (retdec > 0 && encoderPosition == 1)
			{
				_interpolatedposition = float(encoderResolution) + ((-1) * retdec);
			}
			else
			{
				_interpolatedposition = float(encoderPosition - 1) + ((-1) * retdec);
			}
		}
		else if (startposition == 4)
		{ // sensor1 is in neutral
		...
		recaliberate_startPosition();
		// cap resolution count
		_interpolatedposition *= float(setresolution);
		// damp count value
		if (int(_interpolatedposition) == int(prev_interpolatedposition))
		{
			_interpolatedposition = (_interpolatedposition * alpha) + (prev_interpolatedposition * (1 - alpha));
			float tempprevpos = prev_interpolatedposition;
		}
		prev_interpolatedposition = _interpolatedposition;
		if (exAbsolutePos)
		{
		...
		{
			if (int(_interpolatedposition) > tempcount)
			{
				if (tempcount == 0)
				{
					tempcount = int(_interpolatedposition);
					return -1.0;
				}
				tempcount = int(_interpolatedposition);
				return 1.0;
			}
			else if (int(_interpolatedposition) < tempcount)
			{
				if (tempcount == ((encoderResolution - 1) * setresolution))
				{
					tempcount = int(_interpolatedposition);
					return 1.0;
				}

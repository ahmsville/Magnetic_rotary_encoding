...
private:
	...
	int countt = 0, distance = 0, delta_sensor1 = 0, delta_sensor2 = 0;
	int SensorINTpin[4] = { 0, 0, 0, 0 };
	bool exAbsolutePos = false;
	float _interpolatedposition = 0;
	float prev_interpolatedposition = 0;
...
public:
	...
	float detect_rotationHR(); // rotation encoding function
	...
	void setExtendedAbsolutePositioning(bool bres);
	float getinterpolatedStep(int upperlim, int lowerlim, int sdelta);
};
